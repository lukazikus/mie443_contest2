...
//int findPic(cv::Mat& imgTransport, cv::Mat& imgs_track, int iteration){
  ...
  if(!video.empty()){
    ...
    // tag 1
    imgs_track = imread( "/home/andrew/catkin_ws/src/mie443_contest2/pics/tag1.jpg", IMREAD_GRAYSCALE );
    SSD1 = feature2D_homography(imgs_track, video);
    //cv::threshold(imgs_track, imgs_bw, 128.0, 255.0, THRESH_BINARY);
    //double SSD1 = feature2D_homography(imgs_bw, video_bw);

    // tag 2
    imgs_track = imread( "/home/andrew/catkin_ws/src/mie443_contest2/pics/tag2.jpg", IMREAD_GRAYSCALE );
    SSD2 = feature2D_homography(imgs_track, video);
    //cv::threshold(imgs_track, imgs_bw, 128.0, 255.0, THRESH_BINARY);
    //double SSD2 = feature2D_homography(imgs_bw, video_bw);

    // tag 3
    imgs_track = imread( "/home/andrew/catkin_ws/src/mie443_contest2/pics/tag3.jpg", IMREAD_GRAYSCALE );
    SSD3 = feature2D_homography(imgs_track, video);
    //cv::threshold(imgs_track, imgs_bw, 128.0, 255.0, THRESH_BINARY);
    ...
    if (SSD1 < error_threshold || SSD2 < error_threshold || SSD3 < error_threshold){
      if (SSD1 < error_threshold && SSD1 == min(SSD1, min(SSD2, SSD3))){
        ...
        printf("This is cinnamon roast crunch!!!\n");
        foundPic = 2;
    ...
    //change address*******************************************************************************************
    cv::imwrite("/home/andrew/catkin_ws/src/mie443_contest2/src/tag"+ patch::to_string(iteration) +".jpg", video); // CHANGE IMAGE PATH
    cv::waitKey(10);
    video.release();
    cv::waitKey(10);
  }
